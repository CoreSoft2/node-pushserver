...
app.post('/subscribe', function (req, res) {
    ...
    if (config.get('apiToken') == deviceInfo.apiToken){
...
app.post('/unsubscribe', function (req, res) {
    ...
    if (config.get('apiToken') == data.apiToken && data.user) {
        push.unsubscribeUser(data.user);
    } else if (config.get('apiToken') == data.apiToken && data.token) {
...
app.post('/send', function (req, res) {
    var notifs = [req.body];
    var notificationsValid = false;
    
    if (config.get('appToken') == notifs.appToken){
        notificationsValid = sendNotifications(notifs);
    }
...
app.get('/users/:user/associations', function (req, res) {
    if (config.get('appToken') == req.params.appToken){
...
app.delete('/users/:user', function (req, res) {
    if (config.get('appToken') == req.paramas.appToken){
